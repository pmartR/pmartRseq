...
.plot.countFilter <- function(results_object, breaks=100, max_count=NULL, min_num=NULL, min_samp=NULL, plot_title=NULL, x_lab=NULL, y_lab=NULL) {
  ...
  if(!is.null(attr(results_object, "group_var"))){
    all_counts <- lapply(break_data, function(x){
      temp <- lapply(unique(results_object$Group), function(y){
        data.frame(point=x, sumcount=length(which(subset(results_object, Group==y)[,paste(fn,"OTUs",sep="")] <= x)), Group=y)
      })
      do.call(rbind, temp)
    })
  }else{
  ...
  if(!is.null(attr(results_object, "group_var"))){
    ...
    if(!is.null(min_num)) {
      ...
      num_tested <- lapply(unique(results_object$Group), function(x){
        tmp <- results_object[which(results_object$Group == x),]
